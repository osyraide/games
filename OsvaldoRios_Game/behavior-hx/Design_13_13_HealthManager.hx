...
class Design_13_13_HealthManager extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				if(((actor.getX() <= 179) && (actor.getX() >= 132)))
				{
					_CurrentHealth = asNumber(2);
					propertyChanged("_CurrentHealth", _CurrentHealth);
				}
				else if(((actor.getX() >= 241) && (actor.getX() <= 283)))
				{
					_CurrentHealth = asNumber(1);
					propertyChanged("_CurrentHealth", _CurrentHealth);
				}
				else if(((actor.getX() >= 354) && (actor.getX() <= 400)))
				{
					_CurrentHealth = asNumber(0);
					propertyChanged("_CurrentHealth", _CurrentHealth);
				}
