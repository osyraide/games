...
class Design_13_13_HealthManager extends ActorScript
{
	...
	override public function init()
	{
		...
		addCollisionListener(actor, function(event:Collision, list:Array<Dynamic>):Void
		{
			...
			{
				if((Engine.engine.getGameAttribute("Take Damge") == true))
				{
					Engine.engine.setGameAttribute("Take Damge", false);
					_CurrentHealth = asNumber((_CurrentHealth - 1));
					propertyChanged("_CurrentHealth", _CurrentHealth);
					Engine.engine.setGameAttribute("Debug", _CurrentHealth);
					runLater(1000 * .5, function(timeTask:TimedTask):Void
					{
						Engine.engine.setGameAttribute("Take Damge", true);
					}, actor);
				}
			}
		});
